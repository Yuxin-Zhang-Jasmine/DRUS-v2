...
% -------------------------------------------------------------
%% Simulation of 3-channel synthetic data for DDRM
%--------------------------------------------------------------
close all
% pahntom type
ImgIdx = 2;
...
% noise type (additiveNoisePositive-ap | multiplicativeNoise-mu)
for noisetype = {'ap', 'mu'}
    noisetype = noisetype{1};

    % --Create and save the ground truth
    ...
    if sum(strcmp(noisetype, "mu"))
        x_orig = x_orig.*randn(65536,1);
    end
    x_orig = x_orig/max(max(x_orig));
    save([path 'GT/' noisetype '.mat'], 'x_orig');
    ...
    if strcmp(noisetype, 'mu')
        std_noisey = 0;
    elseif strcmp(noisetype, 'ap')
        std_noisey = 1;
    end
    ...
    o_Hty = single(o_Hty);
    figure; Image_realScale(Ht*y(:,1), 'DAS', scan)    
    
    % save o_Hty as the observation yd and the DAS results and then 
    % use DDRM to do the reconstruction
end
